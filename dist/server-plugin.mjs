...
var CHANGE_MODES = Object.freeze(["single", "network"]);
function parsePrivateIpv4(value) {
  ...
  const isPrivate = PRIVATE_RANGES.some((range) => octets[0] === range.first && octets[1] >= range.secondMin && octets[1] <= range.secondMax);
  if (!isPrivate) {
...
function validateChangeRequest(body) {
...
function runtimeSnapshot(runtime, configuredPort) {
  const port = Number.isInteger(runtime?.port) && runtime.port >= 1 && runtime.port <= 65535 ? runtime.port : configuredPort;
  return {
